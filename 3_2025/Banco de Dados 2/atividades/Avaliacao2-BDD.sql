...
SELECT CodigoCliente, NomeCliente, Endereco, Cidade, UF FROM CLIENTES 
	WHERE UF = 'SP' OR UF = 'SC'; 
...
-- Utilize aliases para exibir o nome das colunas.(0.5 ponto)
	...
	SELECT name AS 'Nome', create_date AS 'Data de Criação'
		FROM sys.tables;
...
-- Catarina. Utilize aliases para exibir o nome das colunas. (1.0 ponto)
	...
	SELECT CodigoCliente AS 'Código', NomeCliente AS 'Nome', Endereco AS 'Endereço', Cidade, UF AS 'Estado' FROM CLIENTES 
		WHERE UF = 'SP' OR UF = 'SC'; 
...
-- banco de dados. Utilize aliases para exibir o nome das colunas. (1.0 ponto)
	...
	SELECT MAX(ValorUnitario) AS 'Maior valor', MIN(ValorUnitario) AS 'Menor valor'
		FROM PRODUTOS;
...
-- para exibir o nome das colunas. (1.0 ponto)
	...
	SELECT NumeroPedido AS 'Número do Pedido', CodigoProduto AS 'Código do Produto', Quantidade AS 'Quantidade de itens' FROM ITEM_DE_PEDIDO
		WHERE quantidade = 35;
...
-- o nome das colunas. (1.0 ponto)
	...
	SELECT CodigoVendedor AS 'Código', NomeVendedor AS 'Nome', Salario AS 'Salário' FROM VENDEDORES
		WHERE NomeVendedor LIKE 'A%' OR NomeVendedor LIKE 'C%' OR NomeVendedor LIKE 'F%' OR NomeVendedor LIKE 'M%'
...
-- Utilize aliases para exibir o nome das colunas. (1.0 ponto)
	...
	COUNT (CodigoProduto) AS 'Total de produtos diferentes',
	SUM (Quantidade) AS 'Total geral de itens'
		FROM ITEM_DE_PEDIDO
...
-- Utilize aliases para exibir o nome das colunas. (1.5 pontos)
	...
	SELECT CodigoVendedor, NomeVendedor, Salario FROM VENDEDORES 
